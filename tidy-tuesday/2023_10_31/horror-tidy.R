...
working_dir <- here::here("tidy-tuesday", "2023_10_31")
...
horror_articles$year <- format(as.Date(horror_articles$published, format="%Y-%m-%d"),"%Y")

ggplot(horror_articles, aes(x=year)) +
  geom_bar()

ggplot(horror_articles, aes(author)) +
  geom_bar()


ggplot(horror_articles, aes(x=year, fill = author)) +
  geom_bar() + 
  labs(fill = "Author",
       x = "Year",
       y = "Amount of Articles",
       title = "Amount of Articles Written each Year by Author",
       caption = "(based on data from tidytuesday)")
